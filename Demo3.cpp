...
int main(int argc, char **argv)
{
  ...
  if (argc > 1)
  {
    ...
    std::cout << "Usage: bgs_demo3 [folder name°" << std::endl;
    return 0;
  }
  ...
  for (int i = 0; i < 100 ; i ++) {
    ...
    if ((dir = opendir (path.c_str())) != NULL) {
      std::cout << "Opened dir." << std::endl;
      while ((ent = readdir (dir)) != NULL) {
        std::string file_path = path + "/" + ent->d_name;
        std::cout << "Reading file " << file_path << std::endl;
        // read next image
        img_input = imread(file_path, IMREAD_COLOR);
        if (img_input.empty()) continue;

        std::cout << "Successfully read image " << file_path << std::endl;
